...
export default function AlphaBetaGame() {
  ...
  const formatInputValue = (key) => {
    const v = userInputs[key];
  ...
  const parseVal = (str) => {
    ...
    // coerce to string so methods exist
  ...
  const showSolution = () => {
    try {
      setErrorMsg(null);
      console.log('showSolution invoked', { nodes, solution });
      const solvedInputs = {};
      Object.keys(nodes).forEach(nodeId => {
        ...
        if (node.type !== 'LEAF' && !solution.prunedNodes.has(nodeId)) {
          ...
          solvedInputs[`${nodeId}_alpha`] = String(formatInf(state.alpha));
          solvedInputs[`${nodeId}_beta`] = String(formatInf(state.beta));
          solvedInputs[`${nodeId}_val`] = String(formatInf(state.value));
        }
      });
      ...
      console.error('Error en showSolution:', err);
      ...
      <div className="bg-black/40 p-4 rounded-xl shadow-lg border border-white/10 flex flex-wrap gap-4 items-center justify-between w-full max-w-6xl mb-4 text-white">
        <div>
          ...
          <p className="text-sm text-gray-300 mt-1">
            Haz clic en las líneas para marcar dónde podar. Llena los valores de α, β y valor. Usa <code className="bg-black/50 px-1 rounded">inf</code> y <code className="bg-black/50 px-1 rounded">-inf</code>.
          </p>
        </div>
        <div className="flex gap-3 flex-wrap">
          ...
          <button onClick={checkAnswers} className="px-4 py-2 bg-green-600 hover:bg-green-500 rounded font-semibold transition-colors">
        ...
        <svg viewBox={`0 0 ${WIDTH} ${HEIGHT}`} className="w-full h-auto min-w-[1000px] select-none">
          ...
          {Object.values(nodes).map(node => {
            ...
            return node.children.map(childId => {
              ...
              // Lógica de colores: mostrar feedback solo en checkMode
                  ...
                  <div xmlns="http://www.w3.org/1999/xhtml" className="flex flex-col gap-1 text-white text-sm">
                    <div className="flex items-center drop-shadow-md">
                      ...
                      <input 
                        ...
                        value={formatInputValue(`${node.id}_alpha`)}
                    ...
                    <div className="flex items-center drop-shadow-md">
                      ...
                      <input 
                        ...
                        value={formatInputValue(`${node.id}_beta`)}
